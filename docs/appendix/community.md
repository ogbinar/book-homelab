# Community

_How to connect with other homelabbers._

---

## The Only Community You Need

### Data Engineering Pilipinas (DEP)
**[https://dataengineering.ph/](https://dataengineering.ph/)**

This guide exists because of DEP. This guide is inspired by DEP. This guide is for DEP.

DEP is built around:
- **Learning openly**
- **Sharing knowledge freely**
- **Helping people grow without gatekeeping**

Join their Discord. Attend their meetups. Connect with other Filipino tech enthusiasts. That's it. That's the community.

> **"The best community is the one that lifts you up without asking for anything in return."** — DEP

---

## Why DEP?

> **"The best homelabber is not the one with the most servers. It's the one who helps others build theirs."**

DEP gives you:
- **Troubleshooting help** when you're stuck
- **Inspiration** from other people's setups
- **Hardware deals** — second-hand sales, group buys
- **Accountability** — someone to share progress with
- **Jobs** — many homelabbers get hired through DEP connections

## How to Contribute

This book is **CC BY-SA 4.0** — you can:
- **Share** — Copy and redistribute the material
- **Adapt** — Remix, transform, and build upon it

Just give credit and share your changes under the same license.

**Ways to contribute:**
1. **Fix typos** — Yes, they exist
2. **Add Filipino context** — Your local hardware prices, ISPs, experiences
3. **Translate** — Tagalog, Cebuano, Ilocano, or any language
4. **Write chapters** — Appendices, advanced topics, new services
5. **Share your setup** — Your journey might help someone else start

## The Spirit

> **"No gatekeeping. No elitism. Just people helping people build cool stuff."**

Whether you're running a ₱0 old laptop or a ₱100,000 rack — you belong here.

Welcome to DEP. Welcome to the community. 🇵🇭
